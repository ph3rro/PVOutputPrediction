...
class PVRegressionDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 mode='train',
                 clip_len=16,
                 frame_sample_rate=2, # irrelevant for PV output regression, we already have spacing of 1 minute between frames
                 crop_size=224,
                 short_side_size=256,
                 new_height=256,
                 new_width=340,
        ...
        self.sparse_sample = sparse_sample #arent using for now
        self.args = args
        self.aug = False
...
'''
    ...
    def _aug_frame(self, buffer, args):
        ...
        #buffer = transforms.ToTensor()(buffer) # we already have it in form (16, 224, 224, 3)
        ...
        '''buffer = spatial_sampling( # this scales the image to a random size between 256 and 320, and then crops it to 224
            buffer,
            spatial_idx=-1,
            min_scale=256,
            max_scale=320,
        ...
        '''if self.rand_erase: # we aren't using this because its not good for sky images
    ...
    def load_video(self, sample, sample_rate_scale=1):
        ...
        for i in range(self.num_segment):
            if seg_len <= converted_len: # if the number of frames in the video is <= than the number of frames of video needed to have clip_len frames given sample_rate
                ...
                index = np.concatenate( # padding of the last index/frame of the video to have clip_len frames
                    (index,
